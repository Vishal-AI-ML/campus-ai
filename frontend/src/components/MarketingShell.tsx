...
export default function MarketingShell({ children }: { children: ReactNode }) {
	return (
		<div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
			{/* Header */}
			<header className="sticky top-0 z-30 border-b border-slate-200 dark:border-white/10 bg-white/80 dark:bg-slate-950/80 backdrop-blur">
				<div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4">
					...
					<div className="flex items-center gap-3">
						...
						<Link
							href="/login"
							className="text-sm text-slate-600 dark:text-slate-300 transition hover:text-slate-900 dark:hover:text-white"
						>
							Log in
						</Link>
						<Link
			...
			<footer className="border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-950">
				<div className="mx-auto grid max-w-6xl gap-8 px-6 py-12 sm:grid-cols-2 md:grid-cols-4">
					...
					<div>
						<h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Product</h4>
					...
					<div>
						<h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Solutions</h4>
						<ul className="mt-3 space-y-2 text-sm text-slate-500 dark:text-slate-400">
							{ROLES.map((role) => (
								<li key={role.slug}>
									<Link href={`/for-${role.slug}`} className="hover:text-slate-900 dark:hover:text-white">
										{role.title}
									</Link>
								</li>
							))}
					...
					<div>
						<h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Get started</h4>
						<ul className="mt-3 space-y-2 text-sm text-slate-500 dark:text-slate-400">
							<li>
