...
1. **Structured access log** - one JSON line per HTTP request (method, path,
   status, duration_ms, request_id). Log shippers (HF Space logs, Logtail, etc.)
...
(local dev, CI, or before you sign up) the worker runs exactly as before and the
`sentry_sdk` package is never even imported. Turn it on later by setting
`SENTRY_DSN` in the host env - no code change needed.

Location:
    E:\\campus-ai\\ai-worker\\observability.py   (identical copy in backend/)
"""
